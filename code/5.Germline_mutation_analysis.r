...
##   - BH correction across all tested variants
  ...
  germ_variants <- read_tsv(file.path(dat.path, "mutation", "germline_variant_summary.tsv.gz"))
  germ_patients <- read_tsv(file.path(dat.path, "mutation", "germline_patient_summary.tsv.gz"))

  MIN_ALT <- 3

  nrow(germ_patients) # 97 patients 
  nrow(germ_variants) # Unique 1738 LP/P variants 
  sum(germ_variants$n_carriers >= MIN_ALT, na.rm = TRUE) # 374 variants 

  # germline mutation burden with age (like TMB x age)
  sp_burden <- cor.test(germ_patients$age, germ_patients$lp_burden, # range of lp_burden is 38 -113; minimum 38 germline variants and maximum 113 variants carry
                      method = "spearman", exact = FALSE)
  cat(sprintf("\nGermline LP/P burden vs Age: rho = %.3f, p = %.4f\n",
            sp_burden$estimate, sp_burden$p.value)) # not significant 
    ...
    cli %>% filter(!is.na(patient_id)) %>% filter(RegID %in% c(1849, 1878, 1906,2020, 2116, 2208))

## CCR2 3'-UTR variant analysis # preliminary analysis identified that CCR2 variant is significantly associated with younger diagnosed patients 

ccr2_df = germ_patients %>% mutate(CCR2_carrier = as.logical(ccr2_carrier))
n_ccr2 = sum(ccr2_df$ccr2_carrier, na.rm = TRUE) # 4 patients have CCR2 mutations 

n_ccr2/nrow(ccr2_df)*100 # 4.2% of patients have CCR2 variant 

age_car    <- na.omit(ccr2_df$age[ccr2_df$CCR2_carrier])
age_noncar <- na.omit(ccr2_df$age[!ccr2_df$CCR2_carrier])
wt_ccr2    <- wilcox.test(age_car, age_noncar) # p = 0.01537, 

median(age_car) # 35 
length(age_car) # 4
median(age_noncar) #58 
length(age_noncar) # 93 
            
ok_ccr2   <- !is.na(ccr2_df$age)
m_ccr2    <- logistf(ccr2_df$CCR2_carrier[ok_ccr2] ~ ccr2_df$age[ok_ccr2], firth = TRUE) # firth logistic regression used for the binary variable (CCR2 mutant or not) 

age_pname <- grep("age", names(m_ccr2$prob), value = TRUE)[1]
firth_p   <- m_ccr2$prob[age_pname]

m_ccr2$coefficients[age_pname] #negatively correlated, older diagnosed patients are less likely to have CCR2 variant 
firth_p
n_ccr2 
...
RNAFOLD_BIN <-path.expand("~/.conda/envs/rnafold/bin/RNAfold")
...
# negative values MFE(alt-ref) indicates that the alternate seq is more stable than the reference one 
...
# 29/101 (29 characters were changed out of 101 seq pairs )
...
panel_a <- ccr2_df |>
  ...
  scale_colour_manual(values = c("grey65", "black"), guide = "none") +
...
ggsave(file.path(fig.path, "Figure5A.png"),panel_a, width = 8, height = 8, units = "cm", dpi = 300)


## Figure S12 — CCR2 3'-UTR RNA secondary structure arc diagram
    ...
    # spans_variant == TRUE; whether the variant position falls within the span of the arc 
    arc_all   <- bind_rows(
    ...
    variant_marker <- tibble(
        allele = factor(
        c("Reference (C)", "Alternate (T)"),
        levels = c("Reference (C)", "Alternate (T)")
        ),
        x = FLANK + 1L,
        y = -4
    )
    ...
    p_arc <- ggplot() +
      ...
      #           shape = 21, size = 1, colour = "grey30", stroke = 0.2) +
      geom_text(data = seq_bar,
                aes(x = pos, y = -1,label    = base),
                size   =1.5, colour = "black",family = "Arial") +
      scale_colour_manual(values = c("FALSE" = "grey55", "TRUE" = "black"),
                          name = "Spans variant") +
      scale_linewidth_manual(values = c("FALSE" = 0.2, "TRUE" = 0.2),
                            name = "Spans variant") +
      scale_fill_manual(values = c("FALSE" = "grey75", "TRUE" = "black"),
                        name = "Variant site") +
      facet_wrap(~allele, ncol = 1) + 
      geom_point(data = variant_marker,aes(x = x, y = y),inherit.aes = FALSE,shape = 17,size = 1,colour = "black") +
      # annotate("text", x = FLANK + 1L, y = -2, label = "▲", size = 3, colour = "black") +
      expand_limits(y = -2.5) +
      labs(x = "Position in 101 nt window (nt 245 of CCR2 3'UTR at centre)",
          ...
          subtitle = sprintf("Ref MFE: %.2f  |  Alt MFE: %.2f  |  (Alt - Ref) MFE: %.2f kcal/mol",
    ...
    ggsave(file.path(fig.path, "FigureS12.png"),p_arc, width = 16, height = 8, units = "cm", dpi = 300)
